...
class Preferences(
    ...
    private val preferences: ConcurrentHashMap<String, MutableStateFlow<Preference?>> = ConcurrentHashMap()

    suspend fun loadPreference(name: String): Preference? {
    ...
    suspend fun loadAllPreferences(): List<Preference> {
        val result = withContext(Dispatchers.IO) {
    ...
    fun get(name: String): Flow<Preference?> {
        return preferences.getOrPut(name) { MutableStateFlow(runBlocking { loadPreference(name) }) } as Flow<Preference?>
    }
}
